...
void setLCD(uint8_t Value,uint8_t Digit)
{
    //Sets a value from 0-F to a specified Digit,with 1 being the leftmost,4 being the rightmost.  Will not display error if Value is outside of 0-F,but display will not update
	int k;
	if (Digit > 4)
    {
		displayLCDError(0x01);
...
void displayLCDDec(uint16_t Value)
{//Displays a 4 Digit number in decimal
	if (Value > 9999)
    {
		displayLCDError(0x10); //Display "Err" if value is greater than 4 digits
	}
	else
    {
		setLCD(Value/1000,1);
		setLCD((Value - (Value / 1000) * 1000) / 100,2);
		setLCD((Value - (Value / 100) * 100) / 10,3);
		setLCD(Value - (Value / 10) * 10,4);
	}
}//End SegLCD_Display4Digit

void displayLCDHex(uint16_t Value)
{ 
    //Displays a 16 bit number in Hex Format
	setLCD((Value&0xF000)>>12,1);
	setLCD((Value&0x0F00)>>8,2);
	setLCD((Value&0x00F0)>>4,3);
	setLCD((Value&0x000F)>>0,4);
}//End displayLCDHex

void displayLCDTime(uint8_t Value1,uint8_t Value2)
{
    //Displays 2 values separated by a colon
	if ((Value1 > 99) | (Value2 > 99))
    {
		displayLCDError(0x10); //Display "Err" if either value is greater than 2 digits
	}
	else{
		setLCD(Value1 / 10,1);
		setLCD(Value1 % 10,2);
...
void displayLCDError(uint8_t ErrorNum)
{
	...
	if (ErrorNum < 0x10)
    {
		setLCD(ErrorNum,4); //Display ErrorNum in digit 4 if within valid range.  If not,leave blank.
	}
	else
    {
